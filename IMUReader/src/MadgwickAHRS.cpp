...

{
	...
	MadgwickAHRS::MadgwickAHRS(float samplePeriod) : SamplePeriod(samplePeriod), Beta(1)
	...
	MadgwickAHRS::MadgwickAHRS(float samplePeriod, float beta) : SamplePeriod(samplePeriod), Beta(beta)
...

	{
		...
		norm = std::sqrtf(ax * ax + ay * ay + az * az);
		...
		norm = 1 / std::sqrtf(s1 * s1 + s2 * s2 + s3 * s3 + s4 * s4);    // normalise step magnitude
		...
		norm = 1 / std::sqrt(q1 * q1 + q2 * q2 + q3 * q3 + q4 * q4);    // normalise quaternion
